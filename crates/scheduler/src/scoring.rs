...
impl ScoringService {
    pub fn new(challenge_registry: Arc<tokio::sync::RwLock<HashMap<String, ChallengeSpec>>>) -> Self {
        Self {
    ...
    pub async fn trigger_scoring(&self, current_block: u64) -> Result<Vec<ScoringResult>> {
        ...
        for (compose_hash, spec) in challenges.iter() {
            match self.score_challenge(compose_hash, spec).await {
                Ok(result) => {
                    info!("Scoring completed for challenge {}: score={}", compose_hash, result.score);
                    results.push(result);
                }
                Err(e) => {
    ...
    async fn score_challenge(&self, compose_hash: &str, spec: &ChallengeSpec) -> Result<ScoringResult> {
            ...
            .await
        {
            Ok(response) => {
                if response.status().is_success() {
                    ...
                    Err(anyhow::anyhow!("Challenge returned error status: {}", response.status()))
                }
            }
            Err(e) => {
                Err(anyhow::anyhow!("Failed to reach challenge API: {}", e))
            }
        }
    }
}
