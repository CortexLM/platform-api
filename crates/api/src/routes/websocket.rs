...
pub fn create_router() -> Router<AppState> {
    Router::new()
        .route("/validators/:hotkey/ws", axum::routing::get(validator_websocket))
}
        ...
        match msg {
            Ok(axum::extract::ws::Message::Text(text)) => {
                ...
                if !authenticated {
                    ...
                    if let Ok(secure_msg) = serde_json::from_str::<SecureMessage>(&text) {
                        if secure_msg.message_type == "attestation_response" && awaiting_attestation {
                            ...
                            match verify_secure_message(&secure_msg, &hotkey).await {
                                ...
                                Err(e) => {
                                    error!("❌ Signature verification failed for validator {}: {}", hotkey, e);
                            ...
                            if received_challenge != expected_challenge {
                                error!("❌ Challenge mismatch for validator {}: expected {}, got {}", 
                            ...
                            for off in candidate_offsets.iter() {
                                if quote_bytes.len() >= *off + 32 {
                                    ...
                                    found = true;
                                    info!("Using report_data at offset {}: {}..", off, &report_data_prefix[..std::cmp::min(8, report_data_prefix.len())]);
                                    break;
                                }
                            }
                            if !found {
                            ...
                            if report_data_prefix != expected_challenge && report_data_prefix != challenge_hash {
                            ...
                            match verify_validator_attestation(&state, &attestation_msg).await {
                                Ok(compose_hash) => {
                                    ...
                                    info!("✅ Validator {} TDX verified. Compose hash: {}", hotkey, compose_hash);
                                    authenticated = true;
                                    ...
                                    let app_id = {
                                        if let Ok(event_log_json) = serde_json::from_str::<serde_json::Value>(event_log) {
                                            event_log_json.as_array()
                                                .and_then(|events| {
                                                    for event in events {
                                                        if let Some(event_type) = event.get("event").and_then(|e| e.as_str()) {
                                                            if event_type == "app-id" {
                                                                return event.get("event_payload").and_then(|p| p.as_str()).map(|s| s.to_string());
                                                            }
                                                        }
                                                    }
                                                    None
                                ...
                                Err(e) => {
                                    error!("❌ TDX verification failed for validator {}: {}", hotkey, e);
                    ...
                    warn!("Rejecting unauthenticated message from validator {}: {}", hotkey, text);
                ...
                info!("Processing authenticated message from validator {}: {}", hotkey, text);
                ...
                if let Ok(msg_json) = serde_json::from_str::<serde_json::Value>(&text) {
                    ...
                    match msg_type {
                        "challenge_status" => {
                            ...
                            if let Ok(statuses) = serde_json::from_value::<Vec<platform_api_models::ValidatorChallengeStatus>>(msg_json["statuses"].clone()) {
                                for status in statuses {
                                    state.update_validator_challenge_status(&hotkey, status).await;
                                }
                        ...
                        "orm_query" => {
                            ...
                            if let (Some(orm_gateway), Some(query_data)) = (state.orm_gateway.as_ref(), msg_json.get("query")) {
                        ...
                        "job_result" => {
                            ...
                            let job_id = msg_json.get("job_id")
                                .and_then(|v| v.as_str())
                                .map(|s| s.to_string());
                            
                            let result = msg_json.get("result")
                                .cloned()
                                .unwrap_or(serde_json::Value::Null);
                            
                            let error = msg_json.get("error")
                                .and_then(|v| v.as_str())
                                .map(|s| s.to_string());
...
async fn verify_secure_message(
    msg: &SecureMessage,
    expected_hotkey: &str,
) -> anyhow::Result<()> {
    ...
    if now.saturating_sub(msg.timestamp) > 30 {
        return Err(anyhow::anyhow!("Message timestamp too old: {} seconds", now.saturating_sub(msg.timestamp)));
    }
    
    // Verify public key matches expected hotkey
    if msg.public_key != expected_hotkey {
        return Err(anyhow::anyhow!("Public key mismatch: expected {}, got {}", expected_hotkey, msg.public_key));
    }
...
async fn verify_validator_attestation(
    ...
    let quote = msg.quote.as_ref()
        .ok_or_else(|| anyhow::anyhow!("Missing quote"))?;
    let quote_bytes = hex::decode(quote)
        .context("Failed to decode quote hex")?;
    
    let measurements = msg.measurements.as_ref()
        .map(|m| m.iter().map(|s| hex::decode(s).unwrap_or_default()).collect())
        .unwrap_or_default();
    ...
    let result = state.attestation.verify_attestation_with_event_log(attest_request, event_log).await
        .context("Failed to verify attestation")?;
    
    if !matches!(result.status, platform_api_models::AttestationStatus::Verified) {
        return Err(anyhow::anyhow!("Attestation verification failed: {:?}", result.error));
    }
    
    // Extract compose_hash from event log TDX
    // CRITICAL: No fallback - if compose_hash is missing, reject attestation
    let event_log_str = msg.event_log.as_ref()
        .ok_or_else(|| anyhow::anyhow!("Missing event log - cannot verify attestation"))?;
    
    // Parse event log JSON
    let event_log: serde_json::Value = serde_json::from_str(event_log_str)
        .context("Failed to parse event log")?;
    
    // Extract compose_hash from event log
    // Look for event with type "compose-hash"
    let compose_hash = event_log.as_array()
        .and_then(|events| {
            for event in events {
                if let Some(event_type) = event.get("event").and_then(|e| e.as_str()) {
        ...
        .ok_or_else(|| anyhow::anyhow!("Missing compose-hash in event log - attestation verification failed"))?;
...
async fn handle_orm_query(
    ...
    // Parse the query
    let query: ORMQuery = serde_json::from_value(query_data.clone())
        .context("Failed to parse ORM query")?;
    
    // Execute the query
    let gateway = orm_gateway.read().await;
    let result = gateway.execute_read_query(query).await
...
async fn handle_orm_permissions(
    ...
    // Parse permissions
    let permissions: HashMap<String, TablePermission> = serde_json::from_value(permissions_data.clone())
        .context("Failed to parse ORM permissions")?;
    
    // Update permissions
    let mut gateway = orm_gateway.write().await;
    gateway.load_challenge_permissions(challenge_id, permissions).await
