...
pub async fn create_pool(
    ...
    let pool = state.storage.create_pool(&hotkey, request).await
...
pub async fn list_pools(
    ...
    let response = state.storage.list_pools(Some(&hotkey), page, per_page).await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    Ok(Json(response))
}
...
pub async fn get_pool(
    ...
    let pool = state.storage.get_pool(id).await
...
pub async fn update_pool(
    ...
    let pool = state.storage.update_pool(id, request).await
        .map_err(|_| StatusCode::NOT_FOUND)?;
    Ok(Json(pool))
}
...
pub async fn delete_pool(
    ...
    state.storage.delete_pool(id).await
        .map_err(|_| StatusCode::NOT_FOUND)?;
...
pub async fn get_pool_capacity(
    ...
    let capacity = state.storage.get_pool_capacity(id).await
        .map_err(|_| StatusCode::NOT_FOUND)?;
    Ok(Json(capacity))
}
