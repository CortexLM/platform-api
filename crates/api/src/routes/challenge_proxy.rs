    ...
    fn into_response(self) -> Response {
        let (status, message) = match self {
            ...
            SignatureError::InvalidTimestamp => {
                (StatusCode::UNAUTHORIZED, "Timestamp too old or invalid".to_string())
            }
            ...
            SignatureError::HotkeyNotInMetagraph => {
                (StatusCode::FORBIDDEN, "Hotkey not found in metagraph".to_string())
            }
            SignatureError::ChallengeNotFound => {
                (StatusCode::NOT_FOUND, "Challenge not found".to_string())
            }
            SignatureError::CvmUnavailable => {
                (StatusCode::BAD_GATEWAY, "Challenge CVM is not available".to_string())
            }
        };
...
async fn verify_miner_signature(
    ...
    let public_key = sr25519::Public::from_ss58check(miner_hotkey)
        .map_err(|_| SignatureError::InvalidHotkey)?;
...
async fn proxy_to_challenge(
    ...
    let challenge_runner = state.challenge_runner
        .as_ref()
    ...
    if challenge_instance.is_none() {
        warn!(challenge_name = challenge_name, "Challenge not found or not running");
    ...
    // Get CVM API URL
    let cvm_api_url = instance.cvm_api_url
        .as_ref()
        .ok_or_else(|| SignatureError::CvmUnavailable)?;
    
    // Build target URL: {cvm_api_url}/sdk/public/{route_name}
    let target_url = format!("{}/sdk/public/{}", cvm_api_url.trim_end_matches('/'), route_name);
...
async fn handle_challenge_public_route(
    ...
    // Proxy to challenge
    proxy_to_challenge(&state, &challenge_name, &route_name, body_json, &verified_hotkey).await
}
