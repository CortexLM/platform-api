...
pub fn create_router() -> Router<AppState> {
    Router::new()
        ...
        .route("/ui/challenges/:id/review-data", get(get_challenge_review_data))
...
pub async fn exchange_session(
    ...
    let signing_key = EncodingKey::from_secret(secret.as_bytes());
    let session_token = encode(&Header::new(Algorithm::HS256), &session_claims, &signing_key)
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let cookie_value = format!("ui_session={}; Path=/; HttpOnly; SameSite=Lax; Max-Age=900", session_token);
...
pub async fn get_submission_proofs(
    ...
    let proofs = vec![
        ProofReceipt {
            proof: "base64...".to_string(),
            signature: "hex...".to_string(),
            timestamp: Utc::now(),
            validator: "5DD...".to_string(),
            score_hash: "sha256...".to_string(),
        },
    ];
