...
pub fn create_router() -> Router<AppState> {
    Router::new()
        .route("/artifacts/:submission_id", get(download_artifact))
}
...
pub async fn download_artifact(
    ...
    tracing::info!("Download artifact request for submission: {}", submission_id);
    ...
    let claims = state.attestation.verify_token(token)
        .map_err(|e| {
    ...
    let instance_id = claims.get("instance_id")
        .and_then(|v| v.as_str())
        .ok_or_else(|| {
    ...
    let (encrypted_artifact, digest) = state.artifact_storage.get_artifact(submission_id).await
        .map_err(|e| {
