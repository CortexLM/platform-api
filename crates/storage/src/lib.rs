...
pub trait StorageBackend: Send + Sync {
    async fn list_challenges(&self, page: u32, per_page: u32, status: Option<String>, visibility: Option<String>) -> Result<ChallengeListResponse>;
    ...
    async fn list_emission_schedules(&self, _status: Option<String>, _emission_type: Option<String>, _challenge_id: Option<Uuid>) -> Result<Vec<EmissionSchedule>>;
    async fn get_emission_schedule(&self, _id: Uuid) -> Result<EmissionSchedule>;
    async fn create_emission_schedule(&self, _request: CreateEmissionScheduleRequest) -> Result<EmissionSchedule>;
    async fn update_emission_schedule(&self, _id: Uuid, _request: UpdateEmissionScheduleRequest) -> Result<EmissionSchedule>;
    async fn distribute_emission(&self, _id: Uuid, _request: DistributeEmissionRequest) -> Result<()>;
    async fn calculate_emission(&self, _request: CalculateEmissionRequest) -> Result<CalculateEmissionResponse>;
    async fn get_emission_aggregate(&self, _period_start: chrono::DateTime<chrono::Utc>, _period_end: chrono::DateTime<chrono::Utc>) -> Result<EmissionAggregate>;
    ...
    async fn get_emission_report(&self, _period_start: chrono::DateTime<chrono::Utc>, _period_end: chrono::DateTime<chrono::Utc>) -> Result<EmissionReport>;
    
    // Pool methods
    async fn list_pools(&self, validator_hotkey: Option<&str>, page: u32, per_page: u32) -> Result<PoolListResponse>;
    async fn get_pool(&self, id: Uuid) -> Result<Pool>;
    async fn create_pool(&self, validator_hotkey: &str, request: CreatePoolRequest) -> Result<Pool>;
    async fn update_pool(&self, id: Uuid, request: UpdatePoolRequest) -> Result<Pool>;
    async fn delete_pool(&self, id: Uuid) -> Result<()>;
    
    // Node methods
    async fn list_nodes(&self, pool_id: Option<Uuid>, page: u32, per_page: u32) -> Result<NodeListResponse>;
...
impl StorageBackend for MemoryStorageBackend {
    ...
    async fn get_subnet_config(&self) -> Result<SubnetConfig> {
        ...
        config.clone().ok_or_else(|| anyhow::anyhow!("Config not found"))
    }
    ...
    async fn get_validator_emission_metrics(&self, hotkey: &str) -> Result<ValidatorEmissionMetrics> {
    ...
    async fn get_pool(&self, id: Uuid) -> Result<Pool> {
        let pools = self.pools.read().await;
        pools.get(&id).cloned().ok_or_else(|| anyhow::anyhow!("Pool not found"))
    }
    ...
    async fn update_pool(&self, id: Uuid, request: UpdatePoolRequest) -> Result<Pool> {
        let mut pools = self.pools.write().await;
        let pool = pools.get_mut(&id).ok_or_else(|| anyhow::anyhow!("Pool not found"))?;
    ...
    async fn delete_pool(&self, id: Uuid) -> Result<()> {
        let mut pools = self.pools.write().await;
        pools.remove(&id).ok_or_else(|| anyhow::anyhow!("Pool not found"))?;
        Ok(())
    }
    ...
    async fn get_node(&self, id: Uuid) -> Result<Node> {
        let nodes = self.nodes.read().await;
        nodes.get(&id).cloned().ok_or_else(|| anyhow::anyhow!("Node not found"))
    }
    ...
    async fn update_node(&self, id: Uuid, request: UpdateNodeRequest) -> Result<Node> {
        let mut nodes = self.nodes.write().await;
        let node = nodes.get_mut(&id).ok_or_else(|| anyhow::anyhow!("Node not found"))?;
    ...
    async fn delete_node(&self, id: Uuid) -> Result<()> {
        let mut nodes = self.nodes.write().await;
        nodes.remove(&id).ok_or_else(|| anyhow::anyhow!("Node not found"))?;
        Ok(())
    }
    ...
    async fn get_pool_capacity(&self, pool_id: Uuid) -> Result<PoolCapacitySummary> {
        ...
        let healthy_nodes = pool_nodes.iter()
        ...
        let available_memory_gb: u32 = pool_nodes.iter().map(|n| n.capacity.available_memory_gb).sum();
