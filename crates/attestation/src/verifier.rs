...
use dcap_qvl::{verify::verify, collateral};
...
impl TdxVerifier {
    pub fn new(config: AttestationConfig) -> Self {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_secs(config.verification_timeout))
            .build()
            .unwrap_or_default();
    ...
    pub async fn verify_static(&self, request: &AttestationRequest, event_log: Option<&str>) -> Result<VerificationResult> {
        tracing::info!("Verifying TDX attestation using dcap-qvl");
        
        let quote = request.quote.as_ref().ok_or_else(|| anyhow::anyhow!("Missing quote"))?;
    ...
    fn extract_app_info(&self, event_log: Option<&str>) -> Result<(Option<String>, Option<String>, Option<String>)> {
